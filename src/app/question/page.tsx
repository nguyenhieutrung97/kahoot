...
export default function QuestionPage() {
  ...
  const { submitAnswer, submitMultipleAnswers } = useGameHub({
    onError: (msg) => { // NEW
          ...
          {showResult && topPlayers.length > 0 && (
            <div className="mt-6 bg-white/80 backdrop-blur border border-gray-200 rounded-lg p-4">
              ...
              <ul className="space-y-1 text-sm">
                {topPlayers.map(tp => (
                  <li key={tp.playerId} className="flex justify-between font-medium">
                    <span>{tp.rank}. {tp.userName || tp.name}</span>
                    <span className="font-semibold text-gray-700">{tp.score}</span>
                  </li>
                ))}
              </ul>
